...
void type_R (uint32_t word, RiscvRegister_t* rscv_reg, Log_t* log) {
  ...
  switch (funct3) {
    ...
    case 1: // SLL
      log->instr = "sll";
      printf("Before = %i", rscv_reg->rg[rd]);
      rscv_reg->rg[rd] = rscv_reg->rg[rs1] << rscv_reg->rg[rs2];
      rscv_reg->PC += 4;
      printf("After = %i\n", rscv_reg->rg[rd]);
      break;
    case 2: // SLT
      ...
      if ((int32_t) rscv_reg->rg[rs1] < (int32_t)rscv_reg->rg[rs2]) {
        rscv_reg->rg[rd] = 1;
        rscv_reg->PC += 4;
        printf("Expected 1, actual= %i\n", rscv_reg->rg[rd]);      
        } else {
        rscv_reg->rg[rd] = 0;
        rscv_reg->PC += 4;
        printf("Expected 0, actual= %i\n", rscv_reg->rg[rd]);
      }
      break;
    case 3: // SLTU
...
void type_B (uint32_t word, RiscvRegister_t* rscv_reg, Log_t* log) {
  ...
  switch(funct3) {
    ...
    case 4: // BLT
      ...
      if ((int32_t)rscv_reg->rg[rs1] < (int32_t)rscv_reg->rg[rs2]) {
        rscv_reg->PC = rscv_reg->PC + imm;
        printf("Branch is less\n");
      } else {
        rscv_reg->PC += 4;
        printf("Branch is greater or equal\n");
      }
      break;
    case 5: // BGE
...
void type_I (uint32_t word, RiscvRegister_t* rscv_reg, struct memory *mem, Log_t* log) {
  ...
  switch (funct3) {
    case 0: // LB
      ...
      rscv_reg->PC += 4;
      printf("Expected 72(ASCII for H), actual %i\n", byte);
      break;
    case 1: // LH
      log->instr = "lh";
      ...
      rscv_reg->PC += 4;
      printf("Expected 18515 (ASCII for HO), actual %i\n", byte);
      break;
    case 2: // LW
      log->instr = "lw";
...
void type_S (uint32_t word, RiscvRegister_t* rscv_reg, struct memory *mem, Log_t* log) {
  ...
  switch (funct3) {
    case 0: // SB
      ...
      rscv_reg->PC += 4;
      printf("Stored byte at address %i: %i\n", address, memory_rd_b(mem, address));
      break;
    case 1: // SH
      ...
      rscv_reg->PC += 4;
      printf("Stored halfword at address %i: %i\n", address, memory_rd_b(mem, address));
      break;
    case 2: // SW
      ...
      rscv_reg->PC += 4;
      printf("Stored word at address %i: %i\n", address, memory_rd_b(mem, address));
      break;
  }
}
